...
AABBTree::AABBTree(unsigned long long initialSize) : _rootNodeIndex(AABB_NULL_NODE), _allocatedNodeCount(0), _nextFreeNodeIndex(0), _nodeCapacity(initialSize), _growthSize(initialSize)
{
	_nodes.resize(initialSize);
	for (unsigned long long nodeIndex = 0; nodeIndex < initialSize; nodeIndex++)
	{
...
//std::list<AABBNode>::iterator AABBTree::getIndex(size_t index) {
//	std::list<AABBNode>::iterator it = _nodes.begin();
...
unsigned long long AABBTree::allocateNode()
{
	...
	if (_nextFreeNodeIndex == AABB_NULL_NODE)
	{
		...
		for (unsigned long long nodeIndex = _allocatedNodeCount; nodeIndex < _nodeCapacity; nodeIndex++)
		{
			AABBNode& node = _nodes[nodeIndex];
			node.nextNodeIndex = nodeIndex + 1;
		}
	...
	unsigned long long nodeIndex = _nextFreeNodeIndex;
...
void AABBTree::deallocateNode(unsigned long long nodeIndex)
{
	AABBNode& deallocatedNode = _nodes[nodeIndex];
	//AABBNode& deallocatedNode = getIndex(nodeIndex)->aabb;
...
{
	unsigned long long nodeIndex = allocateNode();
	AABBNode& node = _nodes[nodeIndex];
...
{
	unsigned long long nodeIndex = _objectNodeIndexMap[object];
...
std::forward_list<std::shared_ptr<IAABB>> AABBTree::queryOverlaps(const std::shared_ptr<IAABB>& object) const
{
	...
	while (!stack.empty())
	{
		unsigned long long nodeIndex = stack.top();
		stack.pop();
...
size_t AABBTree::numberIntersections(
...
) const
{
	...
	std::stack<unsigned long long> stack;
	AABB testAabb = object->getAABB();
	Eigen::Vector3f centroid = object->getCentroid();
	centroid = centroid - camera;
	...
	while (!stack.empty())
	{
		...
		const AABBNode& node = _nodes[nodeIndex];
		if (node.aabb.ray_trace(camera, centroid))
		{
			if (node.isLeaf() && node.object != object
				&& !node.aabb.overlaps(testAabb))
			{
				num_overlaps++;
			}
			else
...
std::vector<std::shared_ptr<IAABB>> AABBTree::getListIntersected(
	const::std::shared_ptr<IAABB>& object,
	Eigen::Vector3f& camera
) const
{
	std::vector<std::shared_ptr<IAABB>> nodes_intersected;
	size_t num_overlaps = 0;
	std::stack<unsigned long long> stack;
	AABB testAabb = object->getAABB();
	Eigen::Vector3f centroid = object->getCentroid();
	...
	while (!stack.empty())
	{
		...
		if (node.aabb.ray_trace(camera, centroid))
		{
			if (node.isLeaf() && node.object != object
				&& !node.aabb.overlaps(testAabb))
			{
				nodes_intersected.push_back(node.object);
			}
			else
			{
				stack.push(node.leftNodeIndex);
	...
	return nodes_intersected;
}
	
void AABBTree::insertLeaf(unsigned long long leafNodeIndex)
{
	...
	unsigned long long treeNodeIndex = _rootNodeIndex;
	...
	while (!_nodes[treeNodeIndex].isLeaf())
	{
		...
		unsigned long leftNodeIndex = treeNode.leftNodeIndex;
		unsigned long rightNodeIndex = treeNode.rightNodeIndex;
	...
	unsigned long long leafSiblingIndex = treeNodeIndex;
	AABBNode& leafSibling = _nodes[leafSiblingIndex];
	unsigned long long oldParentIndex = leafSibling.parentNodeIndex;
	unsigned long long newParentIndex = allocateNode();
	AABBNode& newParent = _nodes[newParentIndex];
	newParent.parentNodeIndex = oldParentIndex;
	newParent.aabb = _nodes[leafNodeIndex].aabb.merge(_nodes[leafSiblingIndex].aabb);// leafNode.aabb.merge(leafSibling.aabb);
	...
	_nodes[leafSiblingIndex].parentNodeIndex = newParentIndex;
...
void AABBTree::removeLeaf(unsigned long long leafNodeIndex)
{
	...
	AABBNode& leafNode = _nodes[leafNodeIndex];
	unsigned long long parentNodeIndex = leafNode.parentNodeIndex;
	const AABBNode& parentNode = _nodes[parentNodeIndex];
	unsigned long long grandParentNodeIndex = parentNode.parentNodeIndex;
	unsigned long long siblingNodeIndex = parentNode.leftNodeIndex == leafNodeIndex ? parentNode.rightNodeIndex : parentNode.leftNodeIndex;
...
void AABBTree::updateLeaf(unsigned long long leafNodeIndex, const AABB& newAaab)
{
...
void AABBTree::fixUpwardsTree(unsigned long long treeNodeIndex)
{
